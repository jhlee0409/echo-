  ...
  try {
    ...
    try {
      ...
      const response: ChatResponse = {
        ...
        provider: aiResponse.provider as 'claude' | 'mock',
 ...
 * 채팅 요청 검증
 */
function validateChatRequest(request: any): {
  valid: boolean
  error?: string
  details?: any
} {
  ...
  if (
    !context.companionPersonality ||
    typeof context.companionPersonality !== 'object'
  ) {
 ...
 * 속도 제한 확인
 */
async function checkRateLimit(_userId: string): Promise<{
  allowed: boolean
  limit: number
  remaining: number
  resetTime: number
...
function calculateRelationshipChange(
  ...
  // 긍정적 상호작용 감지
  if (
    userMessage.includes('고마워') ||
    userMessage.includes('좋아') ||
    userMessage.includes('사랑')
  ) {
    levelChange += 0.1
    intimacyChange += 0.05
  }
