...
export class MockProvider implements AIProvider {
  ...
  private emotionResponses = {
    ...
    excited: [...this.responseTemplates.playful, ...this.responseTemplates.curious],
  ...
  async generateResponse(request: AIRequest): Promise<AIResponse> {
    ...
    const mobileOptimized = request.options?.maxTokens && request.options.maxTokens <= 100
    const baseDelay = mobileOptimized ? 50 : 300
    const randomDelay = mobileOptimized ? Math.random() * 200 : Math.random() * 600
    ...
    try {
      ...
      const templates = this.selectResponseTemplates(responseCategory, companionEmotion)
  ...
  private selectResponseTemplates(category: string, emotion: EmotionType): string[] {
    // Combine category templates with emotion-based templates
    const categoryTemplates = this.responseTemplates[category as keyof typeof this.responseTemplates] || this.responseTemplates.casual
    const emotionTemplates = this.emotionResponses[emotion] || this.responseTemplates.casual
