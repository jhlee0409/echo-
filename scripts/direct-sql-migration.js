...
async function finalVerification() {
  ...
  const verifications = [
    {
      name: '데모 사용자',
      test: () => supabase.from('user_profiles').select('username').eq('username', 'demo_user')
    },
    {
      name: '컴패니언',
      test: () => supabase.from('companions').select('name').eq('user_id', '00000000-0000-0000-0000-000000000001')
    },
    {
      name: '메시지',
      test: () => supabase.from('messages').select('content').eq('conversation_session_id', 'demo-session-001')
    }
  ];
