  ...
  async setupGitHooks() {
    try {
      ...
      try {
        ...
        // Add pre-commit hook
        await execAsync('npx husky add .husky/pre-commit "npx lint-staged"', {
          cwd: rootDir,
        })
...
`,
  ...
  async runTypeCheck() {
    ...
    try {
      ...
      this.log(
        'warning',
        'TypeScript type check found issues (this is normal for a fresh install)'
      )
    }
  }
  ...
  async displayWelcomeMessage() {
    console.log('\n' + '='.repeat(60))
    console.log(
      `${colors.magenta}${icons.sparkles} WELCOME TO SOULMATE AI COMPANION GAME! ${icons.sparkles}${colors.reset}`
    )
    console.log('='.repeat(60))

    console.log(
      `${colors.cyan}🎮 Your AI companion RPG is almost ready!${colors.reset}`
    )
    console.log('')

    console.log(`${colors.yellow}📝 Next Steps:${colors.reset}`)
    console.log(
      `${colors.green}1.${colors.reset} Configure API keys in .env.local`
    )
    console.log(
      `   ${colors.cyan}• Get Claude API key from: https://console.anthropic.com/${colors.reset}`
    )
    console.log(
      `   ${colors.cyan}• Set up Supabase project: https://supabase.com/${colors.reset}`
    )
    console.log('')
    ...
    console.log(
      `   ${colors.cyan}npm run health-check${colors.reset} # Run project health check`
    )
    console.log('')
    ...
    console.log(
      `${colors.green}🚀 Ready to build your AI companion! Happy coding!${colors.reset}`
    )
    console.log('='.repeat(60))
  }
  ...
  async run() {
    console.log(
      `${colors.blue}${icons.gear} Running post-installation setup...${colors.reset}\n`
    )
